            ...
            <div class="px-4 py-4 overflow-hidden bg-white shadow-xl sm:rounded-lg">
                ...
                <div class="mb-4">
                    ...
                    <select name="tags[]" id="tags" multiple>
                        ...
                        @foreach ($tags as $row)
                            <option value="{{$row->id}}" @if ($product->tags->contains($row))
                                selected="selected"
                ...
                <div class="mb-4">
                    <label for="tags"
                        class="block mb-2 text-sm font-bold text-gray-700">Tags associadas ao produto</label>
                    @foreach ($product->tags as $row)
                        <span class="mb-2">{{$row->name}}</span> <br>
                    @endforeach
                </div>
                <div>
                    <button type="submit"
